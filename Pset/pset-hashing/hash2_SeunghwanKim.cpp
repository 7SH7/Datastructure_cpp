...
void rehash(Hash *&ht, int new_size)
{
    ...
    // cout << "your code here\n";
    int new_new_size = nextprime(old_size * 2);
    Hash *new_ht = new Hash{new_new_size};

    // rehashing
    // cout << "your code here\n";
    for (int i = 0; i < old_size; i++)
    {
        list<wordcount> &str = ht->hashtable[i];
        for (auto &it : str)
        {
            int index = hashfunction(new_ht, it.first);
...
bool insert(Hash *&ht, string key)
{
    ...
    // plus) if find, then return true!
    // cout << "your code here:" << index << endl;

    for (auto &it : ht->hashtable[index])
    {
        if (it.first == key)
        {
            it.second++;
    ...
    // cout << "your code here\n";
    ht->hashtable[index].push_back({key, 1});
    ht->nelements++;
...
bool erase(Hash *&ht, string key)
{
    ...
    list<wordcount> &str = ht->hashtable[index];
    // if (ht->hashtable[index].size() > 0)
    if (str.size() > 0)
    { // cannot use find() because of comp op
...
list<wordcount> find(Hash *ht, string key)
{
    ...
    { // cannot use find() because of comp op
        ...
        for (auto &it : ht->hashtable[index])
        {
            if (it.first == key)
            {
                list<wordcount> tmp = ht->hashtable[index];
                return tmp;
            }
        }
    }
