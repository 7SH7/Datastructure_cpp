			...
			if (ifs)
			{
				...
				while (ifs >> word)
				{
					if (isdigit(word.front()))
						continue; // exclude the word
					if (ispunct(word.back()))
						word.pop_back(); // erase a punctunation
					if (word.length() == 0)
						continue;		// exclude if empty
...
void show_range(unordered_map<string, int> &ht, vector<int> &bucket_idx, size_t from, size_t to)
{
	...
	for (size_t i = from; i < to; i++)
	{
		...
		for (auto it = ht.begin(idx); it != ht.end(idx); ++it)
...
int main(int argc, char *argv[])
{
	...
	do
	{
		...
		cout << "\tj - insert N\t\t";
		cout << "\tx - erase N\n";
		cout << "\tk - insert by file\t";
		cout << "\tz - erase by file\n";
		cout << "\tf - find \t\t";
		cout << "\tm - max_loadfactor & rehash\n";
		if (show_n == 0)
...
int main()
{
	...
	mymap = {{"a", 1}, {"called", 2}, {"gracia", 3}, {"commissioned", 4}, {"sola", 5}, {"fide", 6}, {"z", 7}};
