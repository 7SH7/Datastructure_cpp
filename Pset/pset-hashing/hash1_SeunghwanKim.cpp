...
// Signed: Kim Seunghwan  遺꾨컲: ECE20010  Student Number: 22000116///
...
int hashfunction(Hash *ht, int key)
{ // hash function for int key
    return (key % ht->tablesize);
}

int hashfunction(Hash *ht, string key)
{ // hash ft for strings
    int value = 0;
    for (auto x : key)
        value = 37 * value + x;
...
void rehash(Hash *&ht)
{
    ...
    {
        list<string> &lst = ht->hashtable[i];
        for (list<string>::iterator it = lst.begin(); it != lst.end(); it++)
        {
            // cout << (*it) << endl;
            int index = hashfunction(new_ht, *it);
            new_ht->hashtable[index].push_back(*it);
        }
    }
    delete[] ht->hashtable;
...
bool insert(Hash *&ht, string key)
{
    ...
    // if (ht->tablesize <= loadfactor(ht))

    if (ht->threshold <= loadfactor(ht))
    {
        // cout << "here i am!!  " << endl;
        rehash(ht);
    }
...
bool erase(Hash *&ht, string key)
{
    ...
    // cout << "your code here\n";
    int index = hashfunction(ht, key);
    list<string> &str = ht->hashtable[index];
    auto it = find(str.begin(), str.end(), key);
    ...
    // */ this code... hmm.. Both codes are ok..
    // Later change both codes..

    // // get the hash value for key and set the iterator variable: it
    // int search_value = hashfunction(ht, key);
    // list<string>::iterator it = ht->hashtable[search_value].begin(); // loop 돌려서..
    // list<string> &str = ht->hashtable[search_value];

    // list<string> find_complete = find(ht, key);

    // // for loop using it, then return true if erased successfully.
    // cout << "your code here\n";
    // // for (it; it != ht->hashtable[search_value].end(); it++)
    // for (it; it != str.end(); it++)
    // {
    //     if (*it == find_complete)
    //     {

    //         ht->hashtable[search_value].erase(it);
    //     }
    // }
