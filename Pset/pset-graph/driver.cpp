...
int main(int argc, const char **argv)
{
	...
	do
	{
		...
		{
			if (printMode & printAdjList)
			{
				ss += printMenu[printAdjList];
				print_adjlist(g);
			}
			if (printMode & printGraph)
			{
				ss += printMenu[printGraph];
				print_graph(g);
			}
			if (printMode & printTablet)
			{
				ss += printMenu[printTablet];
				print_result(g);
			}
			cout << "\n\t" << menuGraph << ss << "  " << graphspecs(g) << endl;
		}

		cout << "\tn - new graph file\t";
		cout << "x - connected(v,w) \n";
		cout << "\td - DFS(v=0)       \t";
		cout << "e - distance(v,w)  \n";
		cout << "\tb - BFS(v=0)       \t";
		cout << "p - path(v,w)      \n";
