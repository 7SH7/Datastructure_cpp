...
graph graph_by_file(string fname)
{
	...
	while (fp)
	{
		...
		stringstream ss(line);
		ss >> beginning;
		if (beginning.length() == 0 || beginning.c_str()[0] != '.')
			break;
		line.push_back('\n');
...
void print_BFS(graph g)
{
	...
	for (int v = 0; v < V(g); v++)
		cout << setw(4) << g->BFS0[v];
	cout << endl;

	cout << "\t    DistTo[0.." + to_string(V(g) - 1) + "] = ";
	for (int v = 0; v < V(g); v++)
		cout << setw(4) << g->distTo[v];
	cout << endl;

	cout << "\tBFS parent[0.." + to_string(V(g) - 1) + "] = ";
	for (int v = 0; v < V(g); v++)
		cout << setw(4) << g->parentBFS[v];
	cout << endl;
}
	...
	while (!que.empty())
	{
		...
		for (gnode w = g->adj[cur].next; w; w = w->next)
		{
			if (!g->marked[w->item])
			{
				...
				// g->distTo[w->item] = distTo(g, cur, w->item) + 1;
			}
		}
	}
...
void DFSpath(graph g, int v, int w, stack<int> &path)
{
	...
	path.push(w);
	for (int i = 0, j = w; i < V(g); i++)
	{
		if (g->parentDFS[j] == v)
		{
			path.push(g->parentDFS[j]);
			break;
		}
		path.push(g->parentDFS[j]);
		j = g->parentDFS[j];
