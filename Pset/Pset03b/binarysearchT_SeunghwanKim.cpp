...
int binarysearch(T* data, int key, int lo, int hi) {
	...
	{
		if (data[mid] < key && key < data[mid + 1])  return mid;
		return binarysearch(data, key, lo, hi = mid - 1);
	}
	else if (data[mid] < key) {
		if (key < data[mid + 1] && data[mid] < key) return mid + 1;
		return binarysearch(data, key, lo = mid + 1, hi);
	}
...
void binarysearch(T* list, int size) {
	...
	// cout << "your code here \n";
	if (key == list[idx])   cout << (T)list[idx] << "         is @list[" << idx << "]\n";
	else if (key != list[idx])   cout << (T)key << "         is not  @list[" << idx << "]\n";
...
int main(int argc, char *argv[]) {
	...
	// int list[] = { 0, 1, 4, 6 };
	int list[] = { 1,1,1,1,1,3, 5, 6, 8, 9, 11 };
