...
// Given a binary tree, its nodes are printed in level-order.
void treeprint_levelorder(tree root, int level, bool real)
{
	DPRINT(cout << ">treeprint_levelorder lv=" << level << endl;);
	if (root == nullptr)
		return;
	...
	{
		treeprint_levelorder(root->left, level - 1, true);
		treeprint_levelorder(root->right, level - 1, true);
	}
	DPRINT(cout << "<treeprint_levelorder lv=" << level << endl;);
}
...
void treeprint_levelorder_tasty(tree root)
{
	...
	if (h < 7)
		return treeprint_levelorder(root);
	...
	for (int i = h - 2; i <= h; i++)
	{
		...
		cnt = 0;
		vec.clear();
		treeprint_levelorder(root, i, true);
		if (vec.size() > 20)
			show_vector(vec, 20);
		else
			treeprint_levelorder(root, i);
