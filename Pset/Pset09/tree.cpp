...
int degree(tree t) // degree는 하나의 node의 간선의 개수를 반환해주는
{
	int d = 0;
	if (t->left != nullptr)
		d++;
...
bool findPath(tree node, tree x, vector<int> &path)
{
	...
	bool isfind = findPath(tmp->left, x, path) || findPath(tmp->right, x, path);
...
tree maximumBT(tree node)
{
	...
	if (x != nullptr && x->key > max->key)
		max = x;
	if (y != nullptr && y->key > max->key)
		max = y;
	return max;
}
...
{ // returns min node
	if (node == nullptr)
		return node;
	// cout << "your code here1\n";
	tree min = node;
	tree x = minimumBT(node->left);
	// cout << "your code here2\n";
	tree y = minimumBT(node->right);
	if (x != nullptr && min->key > x->key)
		min = x;
	if (y != nullptr && min->key > y->key)
		min = y;
	// cout << "your code here3\n";
	return min;
}
...
int balanceFactor(tree node) // 현재 node의 balance가 맞았는지 확인하는 함수! -> 좌 - 우 -> 이걸로 이후에 확인하겠네..
{
	DPRINT(cout << " bf" << endl;);
	if (empty(node))
		return 0;
...
{ // recreation method
	...
	// your code here – recursive buildAVL() calls for left & right from 0 to mid-1 (or mid number of nodes)
	if (n % 2 == 1)
	{
		root->left = buildAVL(v, mid);
		// root->right = buildAVL(v + mid, mid + 1);
		root->right = buildAVL(v + mid + 1, mid);
	}
	else
	{
		root->left = buildAVL(v, mid);
		// root->right = buildAVL(v + mid, mid);
...
tree trimAVL(tree node, int key)
{
	...
	else if (node->left && node->right)
	{
		...
		if (height(node->left) <= height(node->right))
		{
			tree tmp = succ(node);
			node->key = tmp->key;
			node->right = trimAVL(node->right, tmp->key);
		}
		else
		{
			tree tmp = pred(node);
			node->key = tmp->key;
			node->left = trimAVL(node->left, tmp->key);
		}
	}
	else
	{ // node with only one child or no child
		...
		// node->left 있어. node->right는 nullptr;
	...
	DPRINT(if (node != nullptr)
				   cout
			   << "<trimAVL key=" << key << " is done, now rebalance at " << node->key << endl;);
...
// 시작과 끝이 max가 10개.. 보여지는 것의 최대는 10개라는 거구나..
