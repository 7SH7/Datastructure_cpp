...
// list, node -> list는 head와 tail을 갖고 있고, 저 head와 tail은 node로 되어 있고,, insert나 이딴 거 할 때 사용하는 건 모두 node고,, node는 정보를 담고 있는 것이고,,
...
pNode find(pList p, int val)
{
	...
	while (curr->data != val && curr != end(p))
	{
		curr = curr->next;
	}
...
void pop(pList p, int val)
{
	...
	// cout << "your code here\n";
	find(p, val) != end(p) ? erase(find(p, val)) : void();
	DPRINT(cout << "<pop\n";);
}
...
void pop_all(pList p, int val)
{
	...
	for (pNode c = begin(p); c != NULL; c = c->next)
	{
		// cout << "your code here\n";
		if (val == c->prev->data)
			erase(p, c->prev);
...
void push_backN(pList p, int N)
{
	...
	for (int i = 0; i < N; i++)
	{
		push_back(p, rand_extended() % (1 + range));
	}
...
void show(pList p, bool all, int show_n)
{
	...
	for (i = 0, curr = begin(p); curr != end(p); curr = curr->next, i++)
		if (i >= size(p) - show_n)
			cout << " > " << curr->data;
